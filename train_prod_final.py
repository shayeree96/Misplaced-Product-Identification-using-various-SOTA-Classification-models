...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  8 00:12:51 2020
...
prime_list = sorted(set(Y))
prime_dict = {prime_list[i]: i for i in range(0, len(prime_list))}
...
class mydataset():
    ...
    def __init__(self, classification_list, prime_dict, name):
        ...
        with open(classification_list, mode='r') as f:
            ...
            for line in f:
                ...
                self.X.append(path)
                self.Y.append(prime_dict[Prime_Number])
                #

                #if name=='valid':
                    #print("Prime No:",Prime_Number)
...
model = ResNet(depth=50, num_classes=len(prime_dict))
...
def train(model, data_loader, test_loader, beta, cutmix_prob, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batch_num, (feats, target) in enumerate(data_loader):
            ...
            print('E: %d, B: %d / %d' % (epoch, batch_num + 1, len(data_loaderloader)))
        ...
        #lr_scheduler.step(step_size=30, gamma=0.1)
        lr_scheduler.step()
        print('Epoch: ', epoch + 1)
        ...
        torch.save({
            ...
            'Epoch': epoch
        }, 'saved_model_checkpoints/cutmix')
...
# Optimizer
optimizer = torch.optim.SGD(model.parameters(), lr=0.1, weight_decay=1e-4, momentum=0.9, nesterov=False)
